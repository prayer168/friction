...
const [html,css,js,inclineModel]=await Promise.all(['index.html','styles.css','app.js','incline-model.js'].map(file=>readFile(resolve(root,file),'utf8')));
...
const checks=[
  ['七個主頁籤',(html.match(/role="tab" id="tab-/g)||[]).length===7],
  ...
  ['雙場景實驗室',html.includes('id="flatLab"')&&html.includes('id="inclineLab"')&&html.includes('id="angleRange"')&&html.includes('id="inclineTrialBody"')],
  ['斜坡物理模型',inclineModel.includes('downhill>staticMax')&&inclineModel.includes('Math.sqrt(2*acceleration*RAMP_LENGTH)')&&inclineModel.includes('2*LANDING_MU_K*G')],
  ...
  ['來源與查核日期',(html.match(/查核：2026-07-14/g)||[]).length===5],
